 ...
 * @return Void.
 */
static void free_adapter(void *arg, __attribute__((unused)) void *trash) {
  free(arg);
}
 ...
 * @return Void.
 */
static void rule_free_adapter(void *arg, __attribute__((unused)) void *trash) {
  rule_t *rule = arg;
...
static void filterOnTargets(queue_t **rules, char **targets) {
  ...
  for (idx = 0; idx < (int)queue_size(*rules); idx++) {
    ...
    for (tarIdx = 0; targets[tarIdx] != NULL; tarIdx++) {
      if (strcmp(curRule->target, targets[tarIdx]) == 0) {
        ...
        // can't use a size_t for the idx because this forces it negative
        idx--;
        break;
      }
    }
